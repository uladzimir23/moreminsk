...
В hero-блоке и крупных применениях — **подмена `о` в "Море" на иллюминатор** (•):
...
- Создаёт ownable visual quirk
- Легко считывается как порт-холл / иллюминатор / точка-якорь
- Реализуется через простой SVG/inline-CSS, не нужен графический редактор

В MVP делаем **без custom-treatment**, чтобы не блокировать запуск. Добавляем во вторую итерацию после теста на пользователях (А/Б).
